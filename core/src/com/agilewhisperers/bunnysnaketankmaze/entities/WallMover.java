   ...
   public void runObjectScript(float deltaTime) {
      if (getSteerableComponent() != null && arrive.getTarget() != null) {
         ...
         if (accumulator > timer) {
            accumulator = 0;
            timer = MathUtils.random(3, 10);
            bodies.get(MathUtils.random(0, bodies.size)).setTransform((float) ((int) getBody().getBody().getPosition().x + 0.5), (float) ((int) getBody().getBody().getPosition().y + 0.5), 0);
         }
      }
      ...
      for (int i = 0; i < bodies.size; i++) {
         if (((GameObject) bodies.get(i).getUserData()).getID().equals("Wall")) {
            if (bodies.get(i).getPosition().x - 0.5 != 0
               ...
               bodies.get(i).setType(BodyDef.BodyType.KinematicBody);
            }
         }
      }
      bodies = temp;
